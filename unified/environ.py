...
#####################################################################################
#
# Description: Unified IOAI Environment
...
try:
    ...
    print("Warning: YOLO not available. Pose estimation will use dummy model.")
...
class YoloPoseEstimationModel(PoseEstimationModel):
    ...
    def __init__(self, yolo_model_path: str, cad_models_dir: str, camera_matrix: List[float]):
        ...
        if not YOLO_AVAILABLE:
            raise ImportError("YOLO not available. Please install ultralytics.")
...
class IOAIEnv:
    def __init__(self, headless=False, pose_estimation_model: Optional[PoseEstimationModel] = None, scenario_type="grasp"):
        """
        ...
        Args:
            ...
            scenario_type: Type of scenario ("grasp", "nav", "test", "full")
        """
        self.simulator = None
        self.robot = None
        ...
        if self.pose_estimation_model is None and scenario_type in ["grasp", "full"]:
        ...
        # Setup IK solver if needed
        if scenario_type in ["grasp", "full"]:
            self._setup_mink()
    ...
    def _setup_simulator(self, headless=False):
        """Setup the simulator based on scenario type"""
        ...
        if self.scenario_type == "test":
            ...
            sim_config.scenario_config = scenario_config
        else:
            # Add default scene for other scenarios
        ...
        if self.scenario_type in ["grasp", "full", "test"]:
            self._add_cameras()
        
        # Add objects based on scenario type
        if self.scenario_type == "grasp":
            self._add_grasp_objects()
        elif self.scenario_type == "nav":
            self._add_nav_objects()
        elif self.scenario_type == "full":
            self._add_full_objects()
    ...
    def _add_grasp_objects(self):
        """Add objects for grasping scenario"""
        # Add table
    ...
    def _add_nav_objects(self):
        """Add objects for navigation scenario"""
    ...
    def _add_full_objects(self):
        """Add objects for full scenario with all features"""
        # Add table
        table_config = MeshConfig(
            prim_path="/World/Table",
            mjcf_path=Path()
        ...
        wall_configs = [
            ([center_x, center_y+wall_width/2, wall_height / 2], [wall_width, wall_depth, wall_height]),
            ([center_x, center_y-wall_width/2, wall_height / 2], [wall_width, wall_depth, wall_height]),
            ([center_x+wall_width/2, center_y, wall_height / 2], [wall_depth, wall_width, wall_height]),
            ([center_x-wall_width/2, center_y, wall_height / 2], [wall_depth, wall_width, wall_height])
        ]

        for i, (position, scale) in enumerate(wall_configs):
            wall_config = CuboidConfig(
                prim_path=f"/World/Wall{i+1}",
                position=position,
                orientation=[0, 0, 0, 1],
                scale=scale,
            ...
            self.simulator.add_object(wall_config)
        ...
        # Add cones
        cone_positions = [[3, 3, 0.55], [2, 2, 0.55], [1, 1, 0.55]]
        for i, position in enumerate(cone_positions):
            cone_config = MeshConfig(
                prim_path=f"/World/Cone{i+1}",
                mjcf_path=Path()
                .joinpath(self.simulator.synthnova_assets_directory)
                .joinpath("synthnova_assets")
                .joinpath("objects")
            ...
            self.simulator.add_object(cone_config)
    ...
    def _setup_interface(self):
        ...
        galbot_interface_config.modules_manager.enabled_modules.extend([
            "right_arm", "left_arm", "leg", "head", "chassis", "left_gripper", "right_gripper"
        ])

        galbot_interface_config.right_arm.joint_names = [f"{robot_name}/right_arm_joint{i}" for i in range(1, 8)]
        galbot_interface_config.left_arm.joint_names = [f"{robot_name}/left_arm_joint{i}" for i in range(1, 8)]
        galbot_interface_config.leg.joint_names = [f"{robot_name}/leg_joint{i}" for i in range(1, 5)]
        galbot_interface_config.head.joint_names = [f"{robot_name}/head_joint{i}" for i in range(1, 3)]
        ...
        galbot_interface_config.left_gripper.joint_names = [f"{robot_name}/left_gripper_r_knuckle_joint"]
        galbot_interface_config.right_gripper.joint_names = [f"{robot_name}/right_gripper_r_knuckle_joint"]

        # Enable the front camera modules if cameras are present
        if hasattr(self, 'front_head_rgb_camera_path'):
    ...
    def _init_pose(self):
        """Initialize robot pose based on scenario type"""
        if self.scenario_type == "test":
            poses = {
    ...
    def pick_and_place_callback(self):
        ...
        def init_state():
            ...
            return self._move_right_arm_to_pose(robot_pos, robot_ori)
        ...
        def grasp_state():
            """Grasp the object"""
            if self.state_first_entry:
                self.interface.right_gripper.set_gripper_close()
        ...
        def release_state():
            """Release the object"""
            if self.state_first_entry:
                self.interface.right_gripper.set_gripper_open()
                self.grasp_start_time = time.time()
                self.state_first_entry = False
    ...
    def pick_callback(self):
        ...
        def setval():
            ...
            return do_nothing()
        def goshelf():
            return self.follow_path_callback()
        ...
        self.state_machine.add_state(2, "MoveToPick", move_to_pick_state)
        self.state_machine.add_state(3, "Grasp", grasp_state)
        self.state_machine.add_state(4, "ReturnToInit", return_to_init_state)
        self.state_machine.add_state(5, "SetVals", lambda: setval())
        self.state_machine.add_state(6, "Go Shelf", goshelf)
    ...
    def place_callback(self):
        ...
        # Add states to state machine
        self.state_machine.add_state(0, "Init", init_state)
        self.state_machine.add_state(1, "MoveToPrePlace", move_to_pre_place_state)
        self.state_machine.add_state(2, "MoveToPlace", move_to_place_state)
        self.state_machine.add_state(3, "Release", release_state)
        self.state_machine.add_state(4, "ReturnToInit", return_to_init_state)
    ...
    def follow_path_callback(self):
        """Basic path following callback for navigation"""
